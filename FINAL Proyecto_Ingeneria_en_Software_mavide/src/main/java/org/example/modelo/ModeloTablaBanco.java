...
public class ModeloTablaBanco implements TableModel {
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        Banco banco = datos.get(rowIndex);
        switch (columnIndex) {
            case 0:
                banco.setId_banco((int) aValue);
                break;
            case 1:
                banco.setNumero((int) aValue);
                break;
            case 2:
                banco.setUrl((String) aValue);
                break;
            default:
                System.out.println("No se modifica nada");
                return;
        }
        if (!updateBanco(banco)) {
            System.out.println("Error al actualizar el banco: " + banco.getId_banco());
    }
